...
class UserActivityMapping(object):
        ...
        def isInActivity(self,user,activity):
                aLon=activity['longitude']
                aLat=activity['lattitude']
                activeTimes = []
                for oneTime in user:
        ...
        def getSiteActivities(self, siteName):
                L = 200
                start = 0
                res_len = L
                while res_len == L:                       
                        res = json.loads(self.avosManager.getData(siteName ,limit=L, skip=start))['results']
                        res_len = len(res)
                        self.activities = self.activities+res
                        start = start+L
      
        def getActivities(self):
                print 'Getting damai activities ...'
                self.getSiteActivities('damai')
                print 'Done'
                print 'Getting douban activities ...'
                self.getSiteActivities('douban')
                print 'Done'
                print 'Getting huodongxing activities ...'
                self.getSiteActivities('huodongxing')
                print 'Done'
